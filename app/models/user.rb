...
def weekly_wagers
  wager_tracker = 0
  self.picks.each do |pick|
    if pick.wager != nil && (pick.created_at.beginning_of_week == Date.current.beginning_of_week)
...
def last_week_wagers
  wager_tracker = 0
  self.picks.each do |pick|
    if pick.wager != nil && (pick.created_at.beginning_of_week == 1.week.ago.beginning_of_week)
        wager_tracker = wager_tracker + pick.wager
    end
  end
  if wager_tracker == nil
